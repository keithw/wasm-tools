...
impl<T: WasmModuleResources> FuncValidator<T> {
    ...
    pub fn validate(&mut self, body: &FunctionBody<'_>) -> Result<()> {
        ...
        while !reader.eof() {
            ...
            // the operand stack match the operator's arities.
            #[cfg(debug_assertions)]
            let (pop_push_snapshot, arities) = (
            ...
            #[cfg(debug_assertions)]
            {
                let (params, results) = arities.ok_or(format_err!(
                    reader.original_position(),
                    "coult not calculate operator arity"
                ))?;
