...
impl Printer<'_, '_> {
            ...
            match payload {
                ...
                Payload::CustomSection(c) => {
                    ...
                    match c.as_known() {
                        ...
                        _ => match self.print_known_custom_section(c.clone()) {
                            Ok(()) => continue,
                            Err(e) if !e.is::<BinaryReaderError>() => return Err(e),
                            Err(e) => {
                                let msg =
                                    format!("failed to parse custom section `{}`: {e}", c.name());
    ...
    // Component-model definitions can only make backward references.
    // Invalid components can make backward references in the binary format; need to use numeric
    // index in this case to avoid producing malformed text-format output.
    fn print_fwd_idx<K>(&mut self, names: &NamingMap<u32, K>, idx: u32, max_idx: u32) -> Result<()>
    where
        K: NamingNamespace,
    {
        if idx < max_idx {
            ...
            self._print_idx(&HashMap::default(), idx, K::desc())
        }
    }
    ...
    fn print_known_custom_section(&mut self, section: CustomSectionReader<'_>) -> Result<()> {
        let use_color = self.result.supports_async_color();
        ...
        match section.as_known() {
            ...
            KnownCustom::Producers(s) => {
                internal_printer.newline(section.range().start)?;
                internal_printer.print_producers_section(s)
            }
            KnownCustom::Dylink0(s) => {
                internal_printer.newline(section.range().start)?;
                internal_printer.print_dylink0_section(s)
            }
        ...
        self.line = internal_printer.line;
        let text = String::from_utf8(if use_color {
            buf_color.0.into_inner()
        } else {
            buf_nocolor.0.into_inner()
        })
        .expect("invalid UTF-8");
        self.result.write_str(&text)?;
        Ok(())
    }
